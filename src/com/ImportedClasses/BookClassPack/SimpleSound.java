package com.ImportedClasses.BookClassPack;
 ...
 */
@SuppressWarnings({"CopyConstructorMissesField", "ResultOfMethodCallIgnored", "unused", "UnusedAssignment", "rawtypes"})
public class SimpleSound{
  ...
  public SimpleSound(SimpleSound sound)
  {
    ...
    if (sound.buffer != null)
    {
      ...
      System.arraycopy(sound.buffer, 0, this.buffer, 0, sound.buffer.length);
    }
  }
  ...
  {   
    ...
    if (newBufferSize >= 0) System.arraycopy(oldBuffer, (startFrame * frameSize) + 0, newBuffer, 0, newBufferSize);
  ...
  public byte[] getFrame(int frameNum) throws SoundException
  {
    ...
    if (frameSize >= 0) System.arraycopy(buffer, frameNum * frameSize + 0, theFrame, 0, frameSize);
    return theFrame;
  }
  ...
  public void setFrame(int frameNum, byte[] theFrame) throws SoundException
  {
    ...
    if (frameSize >= 0) System.arraycopy(theFrame, 0, buffer, frameNum * frameSize + 0, frameSize);
  }
    ...
    else if(format.getEncoding().equals(AudioFormat.Encoding.ALAW))
    {
      if((sample>Short.MAX_VALUE)||(sample<Short.MIN_VALUE))
        printError("You are trying to set the sample value to: "+
                   sample + ", but the maxismum value for a sample"+
